...
type
  ...
  private
    ...
    procedure MovQtdeProduto(ACodProd: integer; AQtde: Double; AOperac: string);
...
begin
  try
    ...
    if ds_ConsPedido.DataSet.Eof then
      ...
      begin
        ds_ConsCliente.DataSet := Clientes.ClienteModel.ObterNomeCLI(ds_ConsPedido.DataSet.FieldByName('COD_CLIENTE').AsString);

        EDT_Numpedido.Text   := FormatFloat('000000',ds_ConsPedido.DataSet.FieldByName('NUMPEDI').AsInteger);
        EDT_DTEMISSAO.Date   := ds_ConsPedido.DataSet.FieldByName('DATA_EMISSAO').AsDateTime;
        EDT_CodCliente.Text  := ds_ConsPedido.DataSet.FieldByName('COD_CLIENTE').AsString;
        ...
        while not dsRet_Pedido.DataSet.Eof do
          begin
            ds_ConsProduto.DataSet := Produtos.ProdutoModel.ObterProdDesc(dsRet_Pedido.DataSet.FieldByName('ID_PRODUTO').AsString);

            QryPedproduto.Insert;
            QryPedprodutoID.AsInteger         := dsRet_Pedido.DataSet.FieldByName('ID_PRODUTO').AsInteger;
            QryPedprodutoCodProduto.Value     := dsRet_Pedido.DataSet.FieldByName('ID_PRODUTO').AsString;
  ...
  if statusGravaPed = 'dsInsert' then
    begin
     ...
     if not ValidaCodCliente(EDT_CodCliente.Text) then
       begin
         Showmessage('Cliente não cadastrado, favor selecione outro.');
         EDT_CodCliente.Clear;
         EDT_NomeCliente.Clear;
         EDT_CodCliente.SetFocus;
         Abort;
      ...
      if statusGravaPed = 'dsInsert' then
        begin
          ...
          if not ValidaCodProduto(FormatFloat('0000000000000',StrToInt(edt_CodigoProd.Text))) then
            begin
              Showmessage('Produto não cadastrado, favor selecione outro.');
              edt_CodigoProd.SetFocus;
            ...
            begin
              edt_CodigoProd.Text   := ds_ConsProduto.DataSet.FieldByName('ID').AsString;
              EDT_DescrProduto.Text := ds_ConsProduto.DataSet.FieldByName('DESCRICAO').AsString;
              Edt_PrecProd.Text     := ds_ConsProduto.DataSet.FieldByName('PRECO_VENDA').AsString;
            end;
        end;
    end;
  TEdit(Sender).Color := clWindow;
      ...
      try
        ...
        if Application.messageBox('Deseja Gravar o Pedido ?','Confirmação',mb_YesNo+mb_IconInformation+mb_DefButton2) = IdYes then
          begin
            ...
            PedidovendaCab.PedidovendaModel.ID          := PedidovendaCab.PedidovendaModel.GetId;
            PedidovendaCab.PedidovendaModel.NUMPEDIDO   := PedidovendaCab.PedidovendaModel.ID;
            ...
            while not QryPedproduto.Eof do
              begin
                 ...
                 ProdutosPedido.PedidoXProdutosModel.ID            := StrToInt(EDT_Numpedido.Text);
                 ...
                 ProdutosPedido.PedidoXProdutosModel.Salvar;
                 MovQtdeProduto(StrToInt(ProdutosPedido.PedidoXProdutosModel.CODPRODUTO),
                                ProdutosPedido.PedidoXProdutosModel.QUANTIDADE,
                                '-');
                 QryPedproduto.Next;
              end;
            end
            ...
            begin
              ...
              PedidovendaCab.PedidovendaModel.ID          := QryPedprodutoID.AsInteger;
...
procedure TfrmMov_PEDVendas.MovQtdeProduto(ACodProd: integer; AQtde: Double; AOperac: string);
begin  //movimenta estoque
  try
    Produtos:= TProdutoControl.Create;
    Produtos.ProdutoModel.MovimentaEstoque(AQtde, AOperac, ACodProd.tostring());
  finally
    Produtos.Free;
  end;
end;
...
begin
  try
    ...
    Result := dsRet_Pedido.DataSet.FieldByName('NUMPEDI').AsInteger;
    PedidovendaCab.Free;
  end;
end;
